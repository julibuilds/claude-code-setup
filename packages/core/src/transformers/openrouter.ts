 ...
 * Converts between Claude API format and OpenAI-compatible format used by OpenRouter
 */
...
export class OpenRouterTransformer implements Transformer {
	...
	transformRequest(request: ClaudeRequest, context: TransformerContext): any {
		...
		// Add provider routing if specified in options
		if (options?.provider) {
	...
	private convertMessages(messages: ClaudeMessage[]): any[] {
		...
		for (const msg of messages) {
			...
			// Handle tool use (convert to assistant message with tool_calls)
