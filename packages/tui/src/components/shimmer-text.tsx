...
const generatePaletteFromPrimary = (primaryColor: string, size: number): string[] => {
...
/**
 * ShimmerText component - Animated text with color shimmer effect.
 * Creates a wave of colors that cycles through the text characters.
 */
export function ShimmerText({
	...
	const generateColors = (length: number, colorPalette: string[]): string[] => {
		...
		if (colorPalette.length === 1) {
			return Array.from({ length }, () => colorPalette[0] ?? "#ffffff");
		}
		...
		for (let i = 0; i < length; i++) {
			...
			generatedColors.push(colorPalette[colorIndex] ?? "#ffffff");
		}
	...
	const generatedColors = useMemo(() => generateColors(numChars, palette), [numChars, palette]);
	const attributes = useMemo(() => generateAttributes(numChars), [numChars]);

	const parts: { text: string; color: string; attr: number }[] = [];
	let currentColor = generatedColors[0] ?? "#ffffff";
	let currentAttr = attributes[0] ?? TextAttributes.NONE;
	let currentText = "";

	chars.forEach((char, index) => {
		const phase = (pulse - index + numChars) % numChars;
		const charColor = generatedColors[phase] ?? "#ffffff";
