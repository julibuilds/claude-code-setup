...
function parseConfig(env: Env): AppConfig {
  ...
  if (config.Providers) {
    for (const provider of config.Providers) {
      console.log(
        `Provider ${provider.name}: api_key=${provider.api_key?.substring(
          0,
          10
        )}...`
      );
      ...
      if (provider.api_key === "$OPENROUTER_API_KEY") {
        if (env.OPENROUTER_API_KEY) {
          ...
          console.error(
            `✗ OPENROUTER_API_KEY not found in env for ${provider.name}`
          );
        }
      }
      if (provider.api_key === "$DEEPSEEK_API_KEY") {
        if (env.DEEPSEEK_API_KEY) {
          ...
          console.error(
            `✗ DEEPSEEK_API_KEY not found in env for ${provider.name}`
          );
        }
      }
      if (provider.api_key === "$GEMINI_API_KEY") {
        if (env.GEMINI_API_KEY) {
          ...
          console.error(
            `✗ GEMINI_API_KEY not found in env for ${provider.name}`
          );
        }
      }
    }
